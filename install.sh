...
echo -e "\n${YELLOW}📦 กำลังย้ายไฟล์ uninstallalldl.sh ไปที่ HOME...${RESET}"

if [ -f ~/alldl-yt-dlp/uninstallalldl.sh ]; then
    if mv ~/alldl-yt-dlp/uninstallalldl.sh ~/; then
        echo -e "${GREEN}✅ ย้ายเรียบร้อยแล้ว${RESET}"
    else
        echo -e "${RED}❌ ย้ายไฟล์ไม่สำเร็จ กรุณาลองใหม่อีกครั้ง${RESET}"
        exit 1
    fi
else
    echo -e "${RED}❌ ไม่พบไฟล์ uninstallalldl.sh ที่ ~/alldl-yt-dlp/ กรุณาติดตั้งใหม่${RESET}"
    exit 1
fi

# ───── ลบไฟล์โฟลเดอร์ชั่วคราว ─────
echo -e "\n${YELLOW}🧹 กำลังลบโฟลเดอร์ติดตั้ง alldl-yt-dlp...${RESET}"

if [ -d ~/alldl-yt-dlp ]; then
    if rm -rf ~/alldl-yt-dlp; then
        echo -e "${GREEN}✅ ลบเสร็จสิ้น${RESET}"
    else
        echo -e "${RED}❌ ลบโฟลเดอร์ไม่สำเร็จ กรุณาลองใหม่อีกครั้ง${RESET}"
        exit 1
    fi
else
    echo -e "${RED}❌ ไม่พบโฟลเดอร์ ~/alldl-yt-dlp/${RESET}"
fi
